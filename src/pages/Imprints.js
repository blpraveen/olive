...
const searchCategory = useCallback((search) => {
  const data = {
      ...
      fetch(apiBaseUrl + `olive/search`, requestOptions)
    .then(response => {
...
const filterCategory = useCallback((filter) => {
    ...
    const data = {
      ...
      fetch(apiBaseUrl + `olive/filter`, requestOptions)
    .then(response => {
      return response.json();
    }).then(result => {
       if(result.status){
  ...
  useEffect(async () => { 
    fetch(apiBaseUrl + `get_olive`+`?page=${active_page}`)
      .then(response => {
        return response.json();
      }).then(result => {
        if(result.status){
  ...
  return (
    ...
    <ToastContainer />
      ...
      <div className="categories__content">
        <Row>
          {/* <<<<<<<<<<<<<<< FILTER SEARCH >>>>>>>>>>>>>>>>>>> */}
          <FilterSearch params={{data:filterCategory,search:searchCategory,showPublish:false}}/>
