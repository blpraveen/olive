...
const searchCategory = useCallback((search) => {
  const data = {
      ...
      fetch(apiBaseUrl + `web_magazine/search`, requestOptions)
    .then(response => {
...
const filterCategory = useCallback((filter) => {
    ...
    const data = {
      ...
      fetch(apiBaseUrl + `web_magazine/filter`, requestOptions)
    .then(response => {
      return response.json();
    }).then(result => {
       if(result.status){
  ...
  useEffect(async () => { 
    fetch(apiBaseUrl + `web_magazine`+`?page=${active_page}`)
      .then(response => {
        return response.json();
      }).then(result => {
        if(result.status){
          ...
          if(result.data.books.length){
            ...
            result.data.books.map((book) => {
                ...
                catBook.push({
                  ...
                  price:book.sale_price,
                  offer_zone:book.offer_zone,
                  downdload_link:book.downdload_link,
                })
            });
            setCategoryBook(catBook);
            ...
            <div className="categories__right">
              ...
              <Row>
                {categoryBook.map((data) => {
                  return (
                    <Col xs="6" sm="4" md="2">
                      <div className="book__item" style={{ textDecoration: "none", color: "inherit" }}>
                        <Link
                        ...
                        <div className="book__item__price__div">
                         <p> download</p>
                          <a target="_blank" href={data.downdload_link}>
                          <GetAppIcon
                            type="button"
